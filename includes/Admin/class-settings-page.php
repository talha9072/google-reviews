...
class SettingsPage {
	...
	private static function render_connection_panel(): void {
		...
		if ( self::connect_service_configured() ) {
			echo '<p><a class="button button-primary button-hero" href="' . esc_url( admin_url( 'admin.php?page=gbrw-settings&gbrw_action=connect' ) ) . '">';
			...
			echo '<p><strong>' . esc_html__( 'Connection is not available yet.', 'google-reviews-widget' ) . '</strong></p>';
			echo '<p>' . esc_html__( 'This build has no connect service configured, and Google Business Profile API access is still pending approval. The button turns on once both are in place.', 'google-reviews-widget' ) . '</p>';
			echo '</div>';
		}
	 ...
	 * @return bool
	 */
	private static function connect_service_configured(): bool {
